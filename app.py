...
(school_name) instead of by municipality (muni_name). A school district can span more than
one municipality, so this gives a different -- and often more relevant, for e.g. school
funding or real-estate-market questions tied to school attendance zones -- slice of the
same underlying data. Everything else (filters, toggles, chart types, Total rows) is
identical to the municipality version.
...
def load_sales(path: str) -> pd.DataFrame:
    raw = pd.read_csv(
        path,
        dtype={"prop_class_at_sale": "string", "sale_date_std": "string", "school_name": "string"},
        low_memory=False,
    )
...
# School district list is drawn from the full (pre-outlier-filter, pre-2026-filter) sample
# so the dropdown options stay stable regardless of those toggles below.
school_choices = [ALL_LABEL] + sorted(sales["school_name"].unique())


# -----------------------------------------------------------------------------
# 3. Helpers: apply the outlier/2026 toggles, and build year-by-school-district profiles
# -----------------------------------------------------------------------------
...
def build_year_stats(df: pd.DataFrame) -> dict:
    ...
    year_stats = (
        count_all.merge(price_all, on=["school_name", "sale_year"], how="outer")
                 .sort_values(["school_name", "sale_year"])
                 .reset_index(drop=True)
    )

    # Year-over-year % change, computed relative to the previous year that has at least
    # one recorded sale for that school district (gap years, if any, are skipped rather
    # than treated as zero).
    yoy_stats = year_stats.sort_values(["school_name", "sale_year"]).copy()
    grp = yoy_stats.groupby("school_name")
    for col, newcol in [
...
def plot_level_price(tx: pd.DataFrame, stats_school: pd.DataFrame, school: str):
    ...
    stats_school = stats_school[stats_school["mean_price"].notna()]
    ...
    if years:
        ...
        mean_by_year = stats_school.set_index("sale_year")["mean_price"].reindex(years)
    ...
    ax.set_title(f"Sale price distribution by year — {school}\n"
...
def plot_level_count(stats_school: pd.DataFrame, school: str):
    """Bar chart of transaction counts by year."""
    fig, ax = plt.subplots(figsize=(9, 5))
    x = stats_school["sale_year"].astype(str)
    ax.bar(x, stats_school["n_tx"], color="#4C72B0", alpha=0.8)
    for xi, v in zip(x, stats_school["n_tx"]):
        ax.text(xi, v, str(int(v)), ha="center", va="bottom", fontsize=8)
    ax.set_title(f"Number of transactions by year — {school}")
    ax.set_xlabel("Sale year")
...
def plot_yoy_price(yoy_school: pd.DataFrame, school: str):
    """YoY % change in mean and median sale price."""
    fig, ax = plt.subplots(figsize=(9, 5))
    d = yoy_school.dropna(subset=["mean_yoy"])
    ...
    ax.set_title(f"YoY % change in sale price — {school}")
    ax.set_xlabel("Sale year")
...
def plot_yoy_count(yoy_school: pd.DataFrame, school: str):
    """YoY % change in transaction count, colored by sign."""
    fig, ax = plt.subplots(figsize=(9, 5))
    d = yoy_school.dropna(subset=["n_tx_yoy"])
    ax.axhline(0, color="gray")
    if len(d):
    ...
    ax.set_title(f"YoY % change in number of transactions — {school}")
    ax.set_xlabel("Sale year")
...
def build_level_table(stats_school: pd.DataFrame, tx_school: pd.DataFrame, total_n: int) -> pd.DataFrame:
    ...
    body = pd.DataFrame({
        "Sale year":       stats_school["sale_year"].astype(str),
        "# Transactions":  stats_school["n_tx"],
        "Mean price":      stats_school["mean_price"].map(fmt_dollar),
        "Median price":    stats_school["median_price"].map(fmt_dollar),
        "25th pctile":     stats_school["p25_price"].map(fmt_dollar),
        "75th pctile":     stats_school["p75_price"].map(fmt_dollar),
    })
    prices = tx_school["sale_price_2026usd"]
    total = pd.DataFrame([{
        "Sale year":      "Total",
...
def build_yoy_table(yoy_school: pd.DataFrame, year_school: pd.DataFrame) -> pd.DataFrame:
    """Year-by-year YoY % change, plus a Total row: cumulative % change from the earliest
    to the latest year of data in the period."""
    body = pd.DataFrame({
        "Sale year":           yoy_school["sale_year"].astype(str),
        "# Tx YoY %":          yoy_school["n_tx_yoy"].map(fmt_pct),
        "Mean price YoY %":    yoy_school["mean_yoy"].map(fmt_pct),
        "Median price YoY %":  yoy_school["median_yoy"].map(fmt_pct),
        "25th pctile YoY %":   yoy_school["p25_yoy"].map(fmt_pct),
        "75th pctile YoY %":   yoy_school["p75_yoy"].map(fmt_pct),
    })
    yr = year_school.sort_values("sale_year")
    if len(yr) > 0:
...
school = st.sidebar.selectbox("School district", school_choices, index=0)
...
st.sidebar.caption(
    ...
    "Box plots show the median (center line), 25th–75th percentile range (box), and "
    "whiskers; the red line/points show the mean. Years with no recorded sales for a "
    "school district are omitted, and YoY change is measured against the most recent "
    "prior year with data. Each table's \"Total\" row covers the full period (overall "
    "stats for the level table; first-to-last cumulative change for the YoY table)."
)
...
tx = fs_price if school == ALL_LABEL else fs_price[fs_price["school_name"] == school]
stats_school = year_stats[year_stats["school_name"] == school].sort_values("sale_year")
yoy_school = yoy_stats[yoy_stats["school_name"] == school].sort_values("sale_year")
total_n = len(fs) if school == ALL_LABEL else len(fs[fs["school_name"] == school])

if not show_yoy:
    fig = plot_level_price(tx, stats_school, school) if measure == "price" else plot_level_count(stats_school, school)
else:
    fig = plot_yoy_price(yoy_school, school) if measure == "price" else plot_yoy_count(yoy_school, school)

st.pyplot(fig)
plt.close(fig)

st.subheader("Underlying year-by-year data")
tbl = build_level_table(stats_school, tx, total_n) if not show_yoy else build_yoy_table(yoy_school, stats_school)
